...
proceedButton.addEventListener('click', async function(event) {
    ...
    // Get the div element
    const div = document.getElementById('raincoat-div');
    ...
    html2canvas(div).then(async function(canvas) {
      ...
      const fileName = `raincoatdesign${randomId}`;
 
      // Create a new Firebase Storage reference for the T-shirt design
      const storageRefTshirt = ref(storage, `CartFolder/Raincoat/${fileName}.png`);
